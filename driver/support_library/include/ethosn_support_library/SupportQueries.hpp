...
// Declares standalone functions to check if a layer known by the library is supported with the given
...
SupportedLevel
    IsConstantSupported(const TensorInfo& info, char* reason = nullptr, size_t reasonMaxLength = g_ReasonMaxLength);
