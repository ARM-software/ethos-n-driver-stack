...
{
    return format == BufferFormat::FCAF_DEEP || format == BufferFormat::FCAF_WIDE;
}
...
DmaOp::DmaOp(BufferFormat transferFormat)
    : Op("DmaOp")
    , m_TransferFormat(transferFormat)
    , m_Offset({ 0, 0, 0, 0 })
{}

DmaOp::DmaOp(const char* debugType, BufferFormat transferFormat)
    : Op(debugType)
    , m_TransferFormat(transferFormat)
    , m_Offset({ 0, 0, 0, 0 })
{}
...
void OpGraph::ReducePackedBoundaryData()
{
    for (Buffer* b : m_Buffers)
    {
            ...
            (b->Sram()->m_PackedBoundaryThickness.left > 8 || b->Sram()->m_PackedBoundaryThickness.right > 8))
        {
            ...
            if (dramBuffer->m_Format != BufferFormat::FCAF_WIDE)
            {
