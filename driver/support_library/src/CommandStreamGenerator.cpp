...
std::string GetIntermediateBufferDebugName(const DramBuffer& b, uint32_t bufferId)
{
    std::stringstream ss;
    ...
    ss << "_" << ToString(b.m_DataType);
    // The zero point is needed to decode FCAF tensors, so is useful to include in the filename.
    ss << "_" << ToString(b.m_QuantizationInfo.GetZeroPoint());
    ss << "_" << ToString(b.m_Format);
    ss << "_" << b.m_TensorShape[0] << "_" << b.m_TensorShape[1] << "_" << b.m_TensorShape[2] << "_"
       << b.m_TensorShape[3];
    ss << ".hex";
    return ss.str();
}
...
CommandStreamGenerator::CommandStreamGenerator(const OpGraph& mergedOpGraph,
...
CommandStreamGenerator::~CommandStreamGenerator()
{}

// Compile a given network and return the compiled network
CompiledOpGraph CommandStreamGenerator::Generate()
{
    ...
    try
    {
        for (uint32_t opIdx = 0; opIdx < m_MergedOpGraph.GetOps().size(); ++opIdx)
        {
            ...
            {
                throw NotSupportedException("Op is not currently supported");
            }
        }
    }
    ...
    std::vector<uint32_t> commandStream =
        BuildCommandStream(agents, scheduler.GetDmaRdCommands(), scheduler.GetDmaWrCommands(),
...
const BufferManager& CommandStreamGenerator::GetBufferManager() const
{
    return m_BufferManager;
}

const OpGraph& CommandStreamGenerator::GetMergedOpGraph() const
{
    return m_MergedOpGraph;
}

const std::unordered_map<DramBuffer*, uint32_t>& CommandStreamGenerator::GetDramBufToBufIdMapping() const
{
    return m_DramBufToBufIdMapping;
}

uint16_t CommandStreamGenerator::AddDramBufferAndCacheId(DramBuffer* inputBuffer, Op* const)
{
    ...
    else
    {
        ...
        else if (inputBuffer->m_BufferType.value() == BufferType::Intermediate)
        {
            ...
            m_BufferManager.SetDebugName(inputBufferId, GetIntermediateBufferDebugName(*inputBuffer, inputBufferId));
        }
...
void CommandStreamGenerator::AddSramOverlapDependencies(uint32_t newDataStart,
...
void CommandStreamGenerator::ProcessDmaOp(DmaOp* const ptrDmaOp, uint32_t opIdx)
{
    ...
    if (inputBuffer->m_Location == Location::Dram && outputBuffer->m_Location == Location::Sram)
    {
        ...
        if (inputBuffer->m_Format != BufferFormat::WEIGHT)
        {
            // cppcheck-suppress assertWithSideEffect
    ...
    else if (inputBuffer->m_Location == Location::Sram && outputBuffer->m_Location == Location::Dram)
    {
        ...
        if (m_DramBufToBufIdMapping.find(outputBuffer->Dram()) == m_DramBufToBufIdMapping.end())
        {
            ...
            m_BufferManager.SetDebugName(outputBufferId,
                                         GetIntermediateBufferDebugName(*outputBuffer->Dram(), outputBufferId));
...
void CommandStreamGenerator::ProcessMceOp(MceOp* const ptrMceOp, uint32_t opIdx)
{
...
void CommandStreamGenerator::ProcessPleOp(PleOp* const ptrPleOp, uint32_t opIdx)
{
...
AgentIdType CommandStreamGenerator::AddIfmStreamerToCommandStream(DmaOp* const ptrOp,
...
{
    assert(inputSramBuffer->m_Format == BufferFormat::NHWCB);
...
AgentIdType CommandStreamGenerator::AddWeightStreamerToCommandStream(DmaOp* const ptrDmaOp)
{
    // Get the input buffer to the Dma Op
    OpGraph::BufferList inputBuffers = m_MergedOpGraph.GetInputs(ptrDmaOp);
...
AgentIdType CommandStreamGenerator::AddMceSchedulerToCommandStream(MceOp* const ptrMceOp,
                                                                   command_stream::PleKernelId pleKernelId)
{
    ...
    mceSchedulerData.blockSize.width  = ethosn::utils::NumericCast<uint8_t>(ptrMceOp->m_BlockConfig.m_Width);
    mceSchedulerData.blockSize.height = ethosn::utils::NumericCast<uint8_t>(ptrMceOp->m_BlockConfig.m_Height);
...
AgentIdType CommandStreamGenerator::AddPleLoaderToCommandStream(PleOp* const ptrPleOp)
{
...
AgentIdType CommandStreamGenerator::AddPleSchedulerToCommandStream(PleOp* const ptrPleOp)
{
    // Get the input buffers to the Ple Op
    OpGraph::BufferList inputBuffers = m_MergedOpGraph.GetInputs(ptrPleOp);
    assert(inputBuffers.size() == 1 || inputBuffers.size() == 2);
...
AgentIdType CommandStreamGenerator::AddOfmStreamerToCommandStream(DmaOp* const ptrOp,
...
void CommandStreamGenerator::AddReadAfterWriteDependency(const AgentType consumerAgentType,
...
void CommandStreamGenerator::AddWriteAfterReadDependency(const AgentType consumerAgentType,
                                                         const AgentIdType consumerAgentId,
                                                         const AgentType producerAgentType,
                                                         const AgentIdType producerAgentId,
                                                         const Op* producerOp)
...
bool CommandStreamGenerator::FillConsumerAgentDependency(Dependency& consumerAgentDependency,
                                                         const command_stream::AgentType consumerAgentType,
                                                         const AgentIdType consumerAgentId,
                                                         const command_stream::AgentType producerAgentType,
                                                         const AgentIdType producerAgentId,
                                                         const Op* producerOp) const
{
    ...
    if (isValid)
    {
        DependencyUtils::CalculateInnerRatio(consumerAgentDependency);

        DependencyUtils::CalculateRemainingAgentDependencies(consumerAgentDependency);
    }
    return isValid;
}

// Private function to fill the dependency data for Write After Read dependencies
bool CommandStreamGenerator::FillProducerAgentDependency(Dependency& producerAgentDependency,
                                                         const command_stream::AgentType consumerAgentType,
                                                         const AgentIdType consumerAgentId,
                                                         const command_stream::AgentType producerAgentType,
                                                         const AgentIdType producerAgentId,
                                                         const Op* producerOp) const
{
    ...
    if (isValid)
    {
        DependencyUtils::CalculateInnerRatio(producerAgentDependency);

        DependencyUtils::CalculateRemainingAgentDependencies(producerAgentDependency);
    }
    return isValid;
}
...
void CommandStreamGenerator::AddLifetimeInfoForIntermediateDramBuffers()
{
