...
bool Part::MceStripesInfo::operator<(const MceStripesInfo& rhs) const
{
...
bool Part::PleStripesInfo::operator<(const PleStripesInfo& rhs) const
{
    if (m_Input < rhs.m_Input)
        return true;
    if (rhs.m_Input < m_Input)
...
bool Part::MemoryStripeInfo::operator<(const MemoryStripeInfo& rhs) const
{
...
bool Part::MemoryStripesInfo::operator<(const MemoryStripesInfo& rhs) const
{
    if (m_Input < rhs.m_Input)
        return true;
    if (rhs.m_Input < m_Input)
...
bool Part::NumMemoryStripes::operator<(const NumMemoryStripes& rhs) const
{
    if (m_Input < rhs.m_Input)
        return true;
    if (rhs.m_Input < m_Input)
...
bool Part::MceAndPleInfo::operator<(const MceAndPleInfo& rhs) const
{
...
bool Part::MceOnlyInfo::operator<(const MceOnlyInfo& rhs) const
{
    if (m_MceCompute < rhs.m_MceCompute)
        return true;
    if (rhs.m_MceCompute < m_MceCompute)
...
bool Part::PleOnlyInfo::operator<(const PleOnlyInfo& rhs) const
{
    if (m_PleCompute < rhs.m_PleCompute)
        return true;
    if (rhs.m_PleCompute < m_PleCompute)
...
bool Part::DmaOnlyInfo::operator<(const DmaOnlyInfo& rhs) const
{
    if (m_Input < rhs.m_Input)
        return true;
    if (rhs.m_Input < m_Input)
...
void AddWeightBuffersAndDmaOpToMceOp(OwnedOpGraph& opGraph,
                                     ...
                                     const Part::NumStripesType& numMemoryWeightStripes,
...
std::pair<Buffer*, Buffer*> Part::AddIdentityMceOpForSubGraph(OwnedOpGraph& opGraph,
                                                              Lifetime lifetime,
                                                              const Part::MceStripesInfo& mceComputeInfo,
...
Buffer* AddPleInBuffer(OwnedOpGraph& opGraph,
                       Part::NumStripesType& numPleInputMemoryStripes,
                       const TensorShape& tensorShape,
...
std::pair<Buffer*, Op*> AddMceToOpGraph(OwnedOpGraph& opGraph,
                                        ...
                                        Lifetime lifetime,
                                        TraversalOrder order,
                                        const Part::MceStripesInfo& mceStripeInfo,
                                        const Part::MemoryStripesInfo& memoryStripesInfo,
                                        Part::NumMemoryStripes& numMemoryStripes,
...
void Part::CreateFormatConversionPlans(Node* node,
                                       DmaOnlyInfo& dmaInfo,
                                       NumMemoryStripes& numMemoryStripes,
                                       TraversalOrder order,
...
void GenerateStripes(Node* node,
                     const HardwareCapabilities& caps,
                     const BlockConfig blockConfig,
                     Part::StripeInfos* outStripeInfos)
{
    using namespace utils;
    ...
    Part::NumStripes numStripesInput;
    Part::NumStripes numStripesOutput;
    Part::NumStripes numStripesWeights;
    Part::NumStripes numStripesPleInput;
    ...
    using NumStripes = Part::NumStripes;
    ...
    auto AddStripeInfos = [&](const TensorShape& mceInputStripe, const TensorShape& mceOutputStripe,
        ...
        {
            Part::MceAndPleInfo mceAndPleInfo;
    ...
    // Try splitting height first
    {
        ...
        Part::NumStripes numStripesWeightsCopy = numStripesWeights;
    ...
    // Split only input in height while the output is full tensor
    {
        ...
        numStripesWeightsCopy.m_Min            = std::min(numStripesWeights.m_Min, 1u);
        numStripesWeightsCopy.m_Max            = std::min(numStripesWeights.m_Max, 1u);
        Part::NumStripes numStripesOutputCopy  = numStripesOutput;
    ...
    //// Try splitting width
    {
        ...
        TensorShape pleOutputStripe          = CreateStripe(node->GetShape(), pleOutputEncoding, caps);
        const TensorShape& outputShape       = node->GetShape();
        Part::NumStripes numStripesInputCopy = numStripesInput;
        ...
        Part::NumStripes numStripesWeightCopy = numStripesWeights;
...
void Part::GenerateFormatConversionPlans(Node* node,
                                         TraversalOrder order,
