...
bool IsCompressed(BufferFormat format);
...
{
public:
    DmaOp(BufferFormat transferFormat);
    DmaOp(const char* debugType, BufferFormat transferFormat);
    virtual DotAttributes GetDotAttributes(DetailLevel) const override;
...
public:
    ...
    TBuilder& AddFormat(BufferFormat format);
...
template <class TBuffer, class TBuilder>
TBuilder& BufferBuilder<TBuffer, TBuilder>::AddFormat(BufferFormat format)
{
...
void BufferBuilder<TBuffer, TBuilder>::ValidateCommon()
{
    ...
    assert((BufferFormat::NHWC <= m_Buffer->m_Format) && (m_Buffer->m_Format <= BufferFormat::FCAF_WIDE));
