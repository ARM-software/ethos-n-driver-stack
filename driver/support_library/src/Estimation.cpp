                                              ...
                                              std::unordered_set<Op*>& unprocessed)
{
    ...
    inputConversionData.isNhwc      = inputBuffer->m_Format == BufferFormat::NHWC;
    ...
    outputConversionData.isNhwc      = outputBuffer->m_Format == BufferFormat::NHWC;
                                    ...
                                    std::unordered_set<Op*>& unprocessed)
{
    ...
    for (uint32_t inputIdx = 0; inputIdx < opGraph.GetInputs(frontOp).size(); ++inputIdx)
    {
        ...
        InputStats stats =
            GetInputStats(*sramInputBuffer->Sram(), weightsTensorInfo.m_Dimensions,
                          dramBuffer != nullptr ? dramBuffer->m_Format : utils::Optional<BufferFormat>{});
    ...
    // Check for a DmaOp afterwards, and use that to calculate output stats
    {
        ...
        OutputStats stats = GetOutputStats(
            *sramOutputBuffer->Sram(), dramBuffer != nullptr ? dramBuffer->m_Format : utils::Optional<BufferFormat>{});
        if (dramBuffer != nullptr)
        {
            stats.m_StripesStats =
