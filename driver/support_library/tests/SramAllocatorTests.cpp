...
TEST_CASE("SramAllocator: Allocate prefer end free")
{
    ...
    bool resFree = sram.Free(res0.second);
...
TEST_CASE("SramAllocator: Free")
{
    ...
    auto test = sram.Free(res.second);
    REQUIRE(test == true);
}
...
TEST_CASE("SramAllocator: Allocate between blocks")
{
    ...
    auto test = sram.Free(res1.second);
    REQUIRE(test == true);
...
TEST_CASE("SramAllocator: Reset")
{
    ...
    sram.Reset();
    auto freeResult = sram.Free(res.second);
