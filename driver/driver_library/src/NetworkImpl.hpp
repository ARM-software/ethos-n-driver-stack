...
#define MAX_ETHOSN_COMPILED_NETWORK_MAJOR_VERSION_SUPPORTED 2
#define MIN_ETHOSN_COMPILED_NETWORK_MAJOR_VERSION_SUPPORTED 2
...
public:
    ...
    BufferInfo(uint32_t id, uint32_t offset, uint32_t size)
        : m_Id(id)
        , m_Offset(offset)
        , m_Size(size)
    {}

    BufferInfo(uint32_t id, uint32_t offset, uint32_t size, std::string debugName)
        : m_Id(id)
        , m_Offset(offset)
        , m_Size(size)
    ...
    {
        return m_Id == rhs.m_Id && m_Offset == rhs.m_Offset && m_Size == rhs.m_Size && m_DebugName == rhs.m_DebugName;
    }
...
protected:
    ...
    void DumpIntermediateBuffersBasedOnEnvVar();
    void DumpIntermediateBuffers();

    virtual std::pair<const char*, size_t> MapIntermediateBuffers()              = 0;
    virtual void UnmapIntermediateBuffers(std::pair<const char*, size_t> mapped) = 0;
