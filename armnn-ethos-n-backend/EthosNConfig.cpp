...
constexpr char EthosNConfig::OFFLINE[];
...
EthosNConfig ReadEthosNConfig()
{
    ...
    if (configFilePath != nullptr)
    {
        ...
        if (!configFile.good())
        {
            throw armnn::Exception(std::string("Failed to load config file from: ") + configFilePath);
        }
        configFile >> config;
    }
    ...
    for (size_t lineNo = 1; std::getline(configFile, line); ++lineNo)
    {
        ...
        else if (m[1].matched)
        {
            ...
            else if (m[1] == armnn::EthosNConfig::PERF_VARIANT_VAR)
            {
                ...
                {
                    throw armnn::Exception("Invalid variant specified on line " + std::to_string(lineNo) + ": " + line +
                                           "\nMust be one of: "
            ...
            else if (m[1] == armnn::EthosNConfig::OFFLINE)
            {
                config.m_Offline = TryConvertToBool(m[2], line, lineNo);
            }
            else
            {
